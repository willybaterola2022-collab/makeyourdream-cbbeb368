...
import { Mic, MicOff, Music, Timer, Volume2 } from "lucide-react";
...
const PracticeRoom = () => {
  ...
  const toggleMetronome = () => {
    if (metronomeOn) { metronome.stop(); } else { metronome.start(bpm); }
    setMetronomeOn(!metronomeOn);
  };

  return (
    <StudioRoom config={{ hero: "piano", title: "Sala de Práctica", subtitle: "Sin presión. Solo tú y tu voz." }}>
      <div className="max-w-lg mx-auto space-y-8 p-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-8 rounded-2xl text-center space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            {pitch?.frequency ? `${Math.round(pitch.frequency)} Hz` : "Esperando señal..."}
          </p>
        ...
        <div className="glass-card p-4 rounded-2xl space-y-3">
          <div className="flex items-center justify-between">
            ...
            <StageButton size="sm" variant={metronomeOn ? "destructive" : "outline"} onClick={toggleMetronome}>
              {metronomeOn ? "Parar" : "Iniciar"}
            </StageButton>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-xs text-muted-foreground w-8">40</span>
            <input type="range" min={40} max={200} value={bpm} onChange={e => { setBpm(+e.target.value); if (metronomeOn) { metronome.stop(); metronome.start(+e.target.value); } }}
        ...
        <div className="flex justify-center">
          <StageButton onClick={isListening ? stopMic : requestMic} size="lg" variant={isListening ? "destructive" : "default"} className="w-full">
